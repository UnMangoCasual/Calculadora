...
// ==================== ppm ====================
function calcPPM(tipo) {
    const masaSoluto = parseFloat(document.getElementById('ppm-masa-soluto').value);
    const masaSolucion = parseFloat(document.getElementById('ppm-masa-solucion').value);
    const ppm = parseFloat(document.getElementById('ppm-value').value);
    let resultado = '';

    if (tipo === 'ppm') {
        if (isNaN(masaSoluto) || isNaN(masaSolucion) || masaSolucion <= 0) {
            alert('❌ Ingresa ambas masas');
            return;
        }
        const calc = (masaSoluto / masaSolucion) * 1000000;
        resultado = `ppm = <strong>${calc.toFixed(3)}</strong>`;
    } 
    else if (tipo === 'masa-soluto') {
        if (isNaN(ppm) || isNaN(masaSolucion) || masaSolucion <= 0) {
            alert('❌ Ingresa ppm y masa solución');
            return;
        }
        const calc = (ppm * masaSolucion) / 1000000;
        resultado = `Masa del soluto = <strong>${calc.toFixed(6)}</strong> g`;
    } 
    else if (tipo === 'masa-solucion') {
        if (isNaN(ppm) || isNaN(masaSoluto) || ppm <= 0) {
            alert('❌ Ingresa ppm y masa soluto');
            return;
        }
        const calc = (masaSoluto * 1000000) / ppm;
        resultado = `Masa de la solución = <strong>${calc.toFixed(3)}</strong> g`;
    }
    document.getElementById('ppm-result').innerHTML = resultado;
}
...
function loadExample(tipo, v1, v2, v3) {
    if (tipo === 'mm') {
        ...
        document.getElementById('ppm-masa-soluto').value = v1 || '';
        document.getElementById('ppm-masa-solucion').value = v2 || '';
        document.getElementById('ppm-value').value = v3 || '';
        calcPPM('ppm');
    } else if (tipo === 'm') {
